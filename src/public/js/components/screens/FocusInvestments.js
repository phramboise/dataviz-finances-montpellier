...
export function FocusSol({
    ...
    yearInvestments &&
    yearInvestments.investments / yearInvestments.expenditures;
    ...
    const focusDetailsDenominator = yearDIDetails
        ? yearDIDetails["DI-1"] + yearDIDetails["DI-2"]
        : NaN;

    return React.createElement(
        "article",
        { className: "focus invest" },
        React.createElement(
            "section",
            {},
        ...
        React.createElement(
            ...
            React.createElement(
                ...
                React.createElement(
                    Markdown,
                    {},
                    `**En ${year}, le Département de la Gironde a investi ${yearInvestments &&
            makeAmountString(yearInvestments.investments)} soit ${(
                        investmentProportion * 100
                    ).toFixed(
                        1
                    )}% de la totalité des dépenses. Ce sont les 1,5 Millions d’habitants en Gironde qui bénéficient directement de ces investissements avec par exemple le réaménagement des routes, la construction de collèges ou encore à l’entretien d’espaces naturels.**`
                ),
            ...
            React.createElement(
                ...
                React.createElement(
                    ...
                    React.createElement(
                        ...
                        { className: "number" },
                        yearInvestments &&
              (yearInvestments.investments / population)
                  .toFixed(2)
                  .replace(".", ",")
                    ),
        ...
        React.createElement(
            ...
            React.createElement(SecundaryTitle, {
                text: "Évolution des dépenses d’investissements de 2013 à 2017"
            }),
          ...
          screenWidth >= 800 + 80
              ...
              : screenWidth - 85 >= 600
                ...
                xs: years,
                ysByX: partitionByYear.map(partition =>
                    partition.map(part => part.partAmount)
                ),
                onBrickClicked: (year, id) => {
                    page(`#!/finance-details/${id}`);
                },
                legendItems:
          thisYearPartition &&
          thisYearPartition
...
export default connect(
    state => {
        ...
        const investmentsByYear = docBudgByYear.map(instruction => {
            ...
            const expenditures = hierAggByPrestationList.find(
                e => e.id === EXPENDITURES
            ).total;
            let investments =
        hierAggByPrestationList.find(e => e.id === "DI-1").total +
        ...
        const partitionByYear = docBudgByYear.map(m52i => {
            ...
            return (
                yearElement &&
        yearElement.children &&
        new List(
            ["DI-1-1", "DI-1-2", "DI-1-3", "DI-1-4", "DI-1-5", "DI-2"].map(
                id => ({
                    contentId: id,
        ...
        */
        ...
        // DI details
        const elementById = docBudgByYear.get(currentYear)
            ? makeElementById(
                hierarchicalAggregated(
                    m52ToAggregated(docBudgByYear.get(currentYear), corrections)
                )
            )
            : undefined;
